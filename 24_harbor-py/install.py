...
def install_cli():
    ...
    cli_source = os.path.join(current_dir, "harbor.py")  # Ahora apunta a harbor.py

    # Crear directorios si no existen
    Path(USER_EXECUTABLE_PATH).mkdir(parents=True, exist_ok=True)

    # Copiar el CLI (harbor.py) con nombre 'harbor'
    cli_path = os.path.join(USER_EXECUTABLE_PATH, "harbor")
    try:
    ...
    print(f"\nInstalación completada {SYSTEM_USER}. Puedes usar el comando 'harbor' desde cualquier lugar")
